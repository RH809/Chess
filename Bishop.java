 ...
 * how a bishop moves in Chess (moving diagonally).
 */
public class Bishop extends Piece{
	 ...
	 * 
	 * creates a new bishop with a unique id
	 */
	public Bishop(int r, int c, int color, ChessBoard board) {
		super(r, c, color,board);
		
		wImage = new ImageIcon("Images_and_Sounds/White_Bishop.png");
		bImage = new ImageIcon("Images_and_Sounds/Black_Bishop.png");
	}
	 ...
	 * @param id - id to copy
	 * 
	 * creates a copy of a bishop using another bishop's id
	 */
	public Bishop(int r, int c, int color, ChessBoard board, int id) {
	...
	/**
	 * @return color + Bishop + coords
	 */
	public String toString() {
		String c;
		if(color == 0) c = "White ";
		else c = "Black ";
		return c + "Bishop " + coords.toString();
	}
	
	/**
	 * sets the legal moves based on what a bishop can do
	 * - can move diagonally until reaching another piece or the end of the board
	 * 
	 ...
	 * 
	 * sets all of the moves that this bishop is checking/attacking
	 */
	...
	public void makeNotation(boolean checked, boolean captured, boolean ended, int oldR, int oldC, int newR, int newC, String sound) {
		...
		for(int i = pieces.size() - 1; i >= 0; i--) {
			if(pieces.get(i) instanceof Bishop && !pieces.get(i).equals(this) && pieces.get(i).getColor() == color && pieces.get(i).getCheckMoves().contains(newCoords)) {
				sameMove = true;
